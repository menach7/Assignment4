...
class FSM(Node):
    ...
    def _short_angle(angle):
        """Normalize angle to be within the range -pi to pi."""
        if angle > math.pi:
    ...
    def _compute_speed(diff, max_speed, min_speed, gain):
        """Compute speed for linear or angular velocity, ensuring smooth movement."""
        speed = abs(diff) * gain
        speed = min(max_speed, max(min_speed, speed))
        return math.copysign(speed, diff)

    def _drive_to_goal(self, goal_x, goal_y,
                       heading0_tol = 0.05,  # Tighter tolerance for heading
                       range_tol = 0.05):    # Tighter tolerance for distance
        ...
        if dist > range_tol:
            ...
            if abs(diff) > heading0_tol:
                twist.angular.z = FSM._compute_speed(diff, 1.0, 0.05, 1.5)  # Sharper angular turn with increased gain
            ...
            twist.linear.x = FSM._compute_speed(dist, 0.8, 0.2, 0.6)  # Increased linear speed for faster movement
            self._publisher.publish(twist)
